...
if selected == 'Home':
    ...
    with colw:
        with st.container(height=450):
            ...
            st.dataframe(pd.read_csv("../data/final_marks.csv").head(2))
    ...
    with colx:
        with st.container(height=450):
            ...
            st.write(":green[DataSnippet]")
            st.dataframe(pd.read_csv("../data/all_job_post.csv").head(2))
            st.write("Taken Career Skills & Knowledge Base data from LLM - Chatgpt to Build Career Profile")
            st.write(":green[DataSnippet]")
            st.dataframe(pd.read_csv("../data/career_skills.csv").head(2))
            st.write(":green[Implementation]")
